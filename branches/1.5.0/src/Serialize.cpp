...
{	
	...
	//загрузка параметра метода перемещени€ окна
	...
	//загрузка параметра метода изменени€ размера окна
	...
	if(CHook::GetHook()->m_bTranspStep==0)
		CHook::GetHook()->m_bTranspStep=10;
	//загрузка параметров позиционировани€ OSD окна
...
void CWorkWnd::SaveSettings()
{
	...
	//сохранение параметра метода перемещени€ окна
	m_ProfileReg.WriteProfileInt(_T("MoveSize"),_T("MoveMethod"),
		CHook::GetHook()->m_MoveMethod);

	//сохранение параметра метода изменени€ размера окна
	...
	//сохранение параметров позиционировани€ OSD окна
	...
	//сохранение параметра обновлени€
